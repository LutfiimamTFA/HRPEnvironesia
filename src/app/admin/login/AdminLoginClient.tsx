...
export default function AdminLoginClient() {
  ...
  useEffect(() => {
    if (!loading && userProfile && userProfile.role !== 'kandidat') {
      // If a push notification or deep link sent the user here while logged
      // out, honor returnUrl instead of always landing on /admin.
          ...
          <div className="flex w-1/2 flex-col items-center px-6 pb-8 pt-2">
            <div className="w-full max-w-[480px]">
              ...
              <p className="mt-3 text-center text-xs text-slate-500 dark:text-slate-600">
                &copy; Environesia Group &mdash; HRP Internal System
              </p>
            </div>
          </div>
        </div>

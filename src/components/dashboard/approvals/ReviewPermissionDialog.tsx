...
const buildTimeline = (submission: PermissionRequest) => {
  const items: { label: string; date?: Date; by?: string; notes?: string }[] =
    [];
  ...
  if (submission.status === "pending_manager" || submission.managerDecisionAt)
    items.push({
      label: "Dikirim ke Atasan",
      date: submission.managerDecisionAt
        ? submission.managerDecisionAt.toDate()
  ...
  if (submission.hrdDecisionAt && submission.status === "approved")
    items.push({
  ...
  if (submission.status === "returned")
    items.push({
      label: "Kembali & Tap-In",
      date: submission.actualReturnAt
        ? submission.actualReturnAt.toDate()
        : undefined,
    });
  if (
    submission.status === "closed" ||
    submission.status === "verified_manager"
  )
    items.push({
      label: "Selesai",
      date: submission.updatedAt ? submission.updatedAt.toDate() : undefined,
    });
  if (submission.status === "reported")
    items.push({
      label: "Pengajuan dilaporkan",
      date: submission.reportedExitAt
        ? submission.reportedExitAt.toDate()
        : undefined,
    });
  return items;
};
          ...
          <div className="p-6 space-y-6">
                ...
                {[
                  { label: "Karyawan Lapor", done: true },
                  {
                    label: "Kembali & Tap-In",
                      ...
                      !!detectedReturnAt || submission.status === "returned",
                    skip: !isOfficeExit,
                  },
                  {
            ...
            <Card className="border border-slate-200 dark:border-slate-800 shadow-none rounded-lg overflow-hidden">
              ...
              <CardContent className="p-5 grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4">
                ...
                <InfoRow
                  label="Divisi / Posisi"
                  value={`${submission.division} / ${submission.positionTitle}`}
                />
                <InfoRow
                  label="Jenis Izin"
                  value={
              ...
              <CardContent className="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <InfoRow
                    ...
                    value={
                      HUMAN_STATUS_LABELS[submission.status] &&
                      submission.status.includes("manager")
                        ? HUMAN_STATUS_LABELS[submission.status](submission)
                        : submission.managerDecisionAt
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {buildTimeline(submission).map((t, i) => (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2">
                      <div className="p-5 border-b md:border-b-0 md:border-r space-y-4">
                        ...
                        <InfoRow
                          ...
                          value={
                            submission.expectedReturnAt
                              ? format(
                                  submission.expectedReturnAt.toDate(),
                                  "HH:mm",
                                )
                              : "-"
                          }
                    ...
                    {submission.needsManagerAttention && (
                      <div className="md:col-span-2 pt-2 border-t border-amber-100 dark:border-amber-900/30 flex items-center gap-2">
                        ...
                        <p className="text-[11px] font-bold text-rose-600 dark:text-rose-400 uppercase italic">
                          Perhatian: Membutuhkan verifikasi khusus dari Manager
                          atas deviasi durasi.
                        </p>
                      </div>
                    )}
              ...
              <Card className="border border-slate-200 dark:border-slate-800 shadow-none rounded-lg overflow-hidden">
                ...
                <CardContent className="p-5 grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4">
                  ...
                  <InfoRow
                    ...
                    value={format(submission.endDate.toDate(), "dd MMM yyyy", {
                      locale: idLocale,
                    })}
                  />
                  <InfoRow

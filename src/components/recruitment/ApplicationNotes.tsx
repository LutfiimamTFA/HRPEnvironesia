...
export function ApplicationNotes({ application, onNoteAdded }: ApplicationNotesProps) {
    ...
    const notes = application.timeline?.filter(event => event.type === 'note_added').sort((a,b) => b.at.toMillis() - a.at.toMillis()) || [];
    ...
    const handleAddNote = async () => {
        ...
        try {
            ...
            toast({ variant: 'destructive', title: 'Gagal menambah catatan', description: error.message });
        } finally {
            setIsSaving(false);
        }
    };
            ...
            <CardContent className="space-y-4">
                <div className="space-y-2">
                    <Textarea 
                        ...
                        rows={3}
                    />
                    <Button onClick={handleAddNote} disabled={isSaving || !newNote.trim()} className="w-full">
                        ...
                        notes.map((note, index) => (
                            <div key={index} className="flex items-start gap-3 text-sm">
                                <Avatar className="h-8 w-8">
                                    <AvatarFallback>{getInitials(note.meta.authorName || '?')}</AvatarFallback>
                                </Avatar>
                                <div className="flex-1 rounded-md bg-muted/50 p-3">
                                    <div className="flex justify-between items-center">
                                        <p className="font-semibold">{note.meta.authorName}</p>
                                        <p className="text-xs text-muted-foreground">{formatDistanceToNow(note.at.toDate(), { addSuffix: true, locale: idLocale })}</p>
                                    </div>

...
export function UnifiedInternalDecision({ application, onStageChange }: UnifiedInternalDecisionProps) {
  ...
  // Pra States
  const [praDecision, setPraDecision] = useState<RecruitmentInternalDecisionStatus | ''>('');
  const [praNote, setPraNote] = useState('');
  
  // Pasca States
  const [pascaDecision, setPascaDecision] = useState<PostInterviewDecisionStatus | ''>('');
  ...
  useEffect(() => {
    if (isPascaAvailable && existingPra && activeTab === 'pra' && !existingPasca) {
      setActiveTab('pasca');
    }
  ...
  const handleSavePra = async () => {
    ...
    if (praNote.length < 5) {
      toast({ variant: 'destructive', title: 'Catatan Wajib', description: 'Mohon berikan alasan/catatan internal.' });
      return;
    }
    ...
    try {
      ...
      if (praDecision === 'lanjut_ke_tahap_selanjutnya' && onStageChange) {
        if (application.status === 'screening' || application.status === 'tes_kepribadian') {
          await onStageChange('interview', 'Kandidat diloloskan evaluasi internal dan maju ke tahap wawancara.');
        }
      }
      ...
      await updateDocumentNonBlocking(appRef, {
        ...
        candidateStatus: praDecision === 'lanjut_ke_tahap_selanjutnya' ? 'lolos' : 'menunggu',
      ...
      if (praDecision === 'lanjut_ke_tahap_selanjutnya') {
          const notifRef = collection(firestore, 'users', application.candidateUid, 'notifications');
          await addDoc(notifRef, {
              userId: application.candidateUid,
              type: 'stage_advanced',
              title: "Anda lolos ke tahap berikutnya",
              message: `Lamaran Anda untuk posisi ${application.jobPosition} telah lolos ke tahap berikutnya.`,
              module: 'recruitment',
  ...
  const handleSavePasca = async () => {
    ...
    try {
        ...
        if (pascaDecision === 'lanjut' && onStageChange) {
            // Option to move to offering if needed, though often manual.
        }
    ...
    <Card className="shadow-2xl border-none rounded-[3rem] bg-[#020617]/50 backdrop-blur-xl overflow-hidden border-t-8 border-violet-500/20 ring-1 ring-white/5 relative">
      ...
      <CardHeader className="bg-violet-500/[0.03] pb-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="flex items-center gap-5">
                ...
                <div>
                    <CardTitle className="text-2xl font-black tracking-tighter uppercase text-slate-100 flex items-center gap-3">
                        Keputusan Internal
                        {(existingPra || existingPasca) && <History className="h-5 w-5 text-indigo-400" />}
                    </CardTitle>
                    <div className="flex items-center gap-4 mt-2 overflow-x-auto whitespace-nowrap pb-1 no-scrollbar">
                        <div className="flex items-center gap-2 px-3 py-1 bg-slate-900/50 rounded-full border border-slate-800">
                             <span className="text-[9px] font-black uppercase text-slate-500 tracking-widest">Pra:</span>
                             <span className={cn(
                                 "text-[10px] font-bold uppercase",
                                 existingPra?.status === 'lanjut_ke_tahap_selanjutnya' ? "text-emerald-400" : existingPra ? "text-rose-400" : "text-slate-600"
                             )}>
                                 {getStatusLabelPra(existingPra?.status)}
                             </span>
                        </div>
                        <div className="flex items-center gap-2 px-3 py-1 bg-slate-900/50 rounded-full border border-slate-800">
                             <span className="text-[9px] font-black uppercase text-slate-500 tracking-widest">Pasca:</span>
                             <span className={cn(
                                 "text-[10px] font-bold uppercase",
                                 existingPasca?.status === 'lanjut' ? "text-teal-400" : existingPasca ? "text-rose-400" : "text-slate-600"
                             )}>
            ...
            <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="bg-slate-900/50 p-1.5 rounded-2xl border border-slate-800">
                <TabsList className="bg-transparent border-0 gap-1 h-12">
                    <TabsTrigger value="pra" className="rounded-xl px-5 font-black uppercase text-[10px] tracking-widest data-[state=active]:bg-indigo-600 data-[state=active]:text-white transition-all h-full">Pra-Wawancara</TabsTrigger>
                    <TabsTrigger 
                        value="pasca" 
      ...
      <CardContent className="p-8 md:p-12">
        <Tabs value={activeTab} className="w-full">
            <TabsContent value="pra" className="m-0 space-y-8 animate-in fade-in slide-in-from-left-4 duration-500 focus-visible:outline-none">
                <DecisionPanel 
                    decision={praDecision}
                    onDecisionChange={setPraDecision}
                    note={praNote}
                    onNoteChange={setPraNote}
                    isLocked={!isHRD || (existingPra?.status === 'lanjut_ke_tahap_selanjutnya' && !['screening', 'tes_kepribadian'].includes(application.status))}
                    isHRD={isHRD}
            ...
            <TabsContent value="pasca" className="m-0 space-y-8 animate-in fade-in slide-in-from-right-4 duration-500 focus-visible:outline-none">
                {!isPascaAvailable ? (
                    <div className="text-center py-20 bg-slate-900/40 rounded-[2.5rem] border-2 border-dashed border-slate-800">
                        ...
                        <h4 className="text-slate-300 font-bold uppercase tracking-widest text-sm">Belum Tersedia</h4>
                        <p className="text-slate-500 text-xs mt-2 italic px-8">Kandidat harus menyelesaikan tahap interview dan tim evaluator harus submit penilaian terlebih dahulu.</p>
                    </div>
                ) : (
                    <DecisionPanel 
...
function DecisionPanel({ type, decision, onDecisionChange, note, onNoteChange, isLocked, isHRD, isSubmitting, onSave, existingData }: DecisionPanelProps) {
    const options = type === 'pra' ? [
        { id: 'lanjut_ke_tahap_selanjutnya' as const, label: 'Lanjut Tahap Selanjutnya', color: 'emerald' as const, icon: CalendarCheck, desc: 'Lolos ke tahap interview' },
        { id: 'pending_internal' as const, label: 'Pending Internal', color: 'amber' as const, icon: PauseCircle, desc: 'Butuh diskusi tim' },
        { id: 'tidak_dilanjutkan_saat_ini' as const, label: 'Tidak Dilanjutkan', color: 'rose' as const, icon: XCircle, desc: 'Gugur seleksi berkas' }
    ] : [
        { id: 'lanjut' as const, label: 'Lanjut ke Tahap Berikutnya', color: 'teal' as const, icon: CalendarCheck, desc: 'Lolos ke tahap offering' },
        { id: 'pending' as const, label: 'Butuh Diskusi Lanjutan', color: 'amber' as const, icon: PauseCircle, desc: 'Butuh rapat internal' },
        { id: 'tidak_lanjut' as const, label: 'Tidak Dilanjutkan', color: 'rose' as const, icon: XCircle, desc: 'Gugur setelah interview' }
    ];
                ...
                {options.map((opt) => {
                    ...
                    return (
                        ...
                        >
                            <div className={cn(
                                "p-3 rounded-xl", 
                                isActive ? `${cfg.badge} text-white shadow-lg` : "bg-slate-800 text-slate-400"
                            )}>
                                <Icon className="h-5 w-5" />
                            </div>
                            <div>
                                <h4 className={cn("text-base font-black uppercase tracking-tight leading-tight", isActive && cfg.text)}>{opt.label}</h4>
                                <p className="text-[10px] font-medium mt-1.5 leading-relaxed opacity-70 italic">{opt.desc}</p>
                            </div>
                        </button>
                    );
                })}
                ...
                <div className="space-y-6">
                    {existingData && (
                        <div className="p-6 rounded-[1.8rem] bg-slate-950/60 border border-slate-800 ring-1 ring-white/5 space-y-4">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Log Keputusan</span>
                            </div>
                            <div className="space-y-2">
                                <div className="space-y-0.5">
                                    <p className="text-[9px] text-slate-500 font-black uppercase">Decided By</p>
                                    <p className="text-sm font-bold text-slate-200">{existingData.decidedByName}</p>
                                </div>
                                <div className="space-y-0.5">
                                    <p className="text-[9px] text-slate-500 font-black uppercase">Timestamp</p>
                                    <p className="text-xs font-medium text-slate-400">
                                        {existingData.decidedAt && (existingData.decidedAt as any).toDate ? format((existingData.decidedAt as any).toDate(), 'dd MMM yyyy, HH:mm') : '-'}
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    {isHRD && !isLocked && (
                        ...
                        >
                            ...
                            {isSubmitting ? 'MENYIMPAN...' : existingData ? 'Update Keputusan' : 'Simpan Keputusan'}
                        </Button>
                    )}
                </div>
            </div>
